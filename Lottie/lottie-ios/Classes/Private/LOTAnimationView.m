...
- (id)initWithDuration:(CGFloat)duration layer:(CALayer *)layer frameRate:(NSNumber *)framerate {
  ...
  if (self) {
    ...
    _layer.speed = 0;
    _layer.timeOffset = 0;
      _layer.beginTime = AVCoreAnimationBeginTimeAtZero;//CACurrentMediaTime();
...
- (void)updateAnimationLayerWithTimeOffset:(CFTimeInterval)timeOffset {
  ...
  _layer.timeOffset = 0;
    _layer.beginTime = AVCoreAnimationBeginTimeAtZero;//0;
  ...
  if (speed == 0) {
    ...
    CFTimeInterval offsetTime =  ((timeOffset != 0) ?
                                  timeOffset / speed :
                                  timeOffset);
      _layer.beginTime = AVCoreAnimationBeginTimeAtZero - offsetTime;//CACurrentMediaTime() - offsetTime;
  }
}
